...
export default function PhotoUpload({ language = "EN" }: PhotoUploadProps) {
  ...
  // Auto-populate name from session guest list
  useEffect(() => {
    const guests = clientSession?.user?.invitation?.Guests;
    if (guests && guests.length > 0 && !uploaderName) {
      setUploaderName(guests[0].Name);
    }
  }, [clientSession]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return (
    <form onSubmit={handleSubmit} className={styles.container}>
      <Field label={labels.nameLabel} required style={{ width: "100%" }}>
        <Input
          value={uploaderName}
      ...
      <Button
        ...
        appearance="primary"
        size="large"
        disabled={loading || files.length === 0 || !uploaderName.trim()}
        icon={loading ? undefined : <ArrowUpload24Regular />}
        className={styles.submitButton}
        style={{
          backgroundColor:
            loading || files.length === 0 || !uploaderName.trim()
              ? "#d1d5db"
              : "#4C4C4C",
          borderColor:
            loading || files.length === 0 || !uploaderName.trim()
